#!/usr/bin/python2.7
"""DS API python script.

DS API script for making authentication, reporting, and conversion API
requests/responses.
...
Usage:
 (1) Run "dsApi.py --login" to generate api credentials.
 ...
 (3) Run "dsApi.py" with the flags --cred, --server, and one of --get, --post,
     or --put. For --post, it reads POST body requests from standard input. The
     script then dumps the API response to standard output.
 (4) Run "dsApi.py --logout" to invalidate the generated credential.
"""
...
def Login():
  """Collects tokens needed to access DoubleClick Search APIs via OAuth2.

  Interactively collect a client id, a client secret, and a refresh token for
  accessing the DoubleClick Search API. Users are asked to enter the client id
  ...
  refresh_token_regex = re.search('refresh_token" : "(.+?)"', output)
  if refresh_token_regex:
...
def GetDSApiCredOrDie(options):
  ...
  if options.cred is not None:
    ...
    print 'Cannot find DS API credentials.'
    sys.exit(-1)
  return cred
...
def GetAccessTokenOrDie(options):
  ...
  Raises:
    Kills the program if the token could not be obtained.
  """
  ...
  output = RunCommand(['curl', '--data',
                       ...
                       'https://accounts.google.com/o/oauth2/token'])
  access_token_regex = re.search('token" : "(.+?)"', output)
  if access_token_regex:
    return access_token_regex.group(1)
  else:
...
def RunREST(options):
  """Executes a DoubleClick Search API call.
...
def Main(argv):
  ...
  parser.add_option(
      ...
      help=('Specifies the DS API credentials, in the format of '
  ...
  if options.login:
    ...
    print ''
    print ('DS API credentials not set.  Please run the script with '
           '--login to generate credentials and set it in the %s'
